...
abstract class AbstractHashTable<T> {
    ...
    public abstract int get_contains_status();
}
...
public class HashTable<T> extends AbstractHashTable<T> {
    ...
    private final int CONTAINS_STATUS_NIL = 0; // команда contains ещё не вызывалась
    private final int CONTAINS_STATUS_OK = 1; // получение значение элемента прошло успешно
    private final int CONTAINS_STATUS_ERR = 2; // искомый элемент отсутсвует в хэш-таблице
    private int put_status;
    private int remove_status;
    private int contains_status;

    public HashTable(){
        slots = initial();
        put_status = PUT_STATUS_NIL;
        remove_status = REMOVE_STATUS_NIL;
        contains_status = CONTAINS_STATUS_NIL;
    }
    ...
    public boolean contains(T value) {
        int index = index(value);
        if (slots[index] == null) {
            contains_status = CONTAINS_STATUS_ERR;
            return false;
        }
        contains_status = CONTAINS_STATUS_OK;
        return true;
    }
    ...
    @Override
    public int get_contains_status() {
